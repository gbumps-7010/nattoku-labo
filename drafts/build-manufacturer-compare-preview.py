...
(function () {
  ...
  function injectMoshimoIframe(container, html) {
    ...
    // もしもは viewport<704px で縦レイアウトになるため、iframe内viewportをPC幅に固定。
    // かんたんリンクのHTML/URL自体は変更しない。枠はもともとの easyLink-box 黒枠に合わせる。
    iframe.srcdoc =
      ...
      "html,body{margin:0;padding:0;background:transparent;color:#0f172a;overflow:hidden}" +
      "body{display:inline-block;min-width:0}" +
      '[id^="msmaflink-"]{display:inline-block;margin:0;max-width:830px}' +
      /* PC横長レイアウトを強制（黒枠= easyLink-box のサイズに追従） */
      "div.easyLink-box{" +
      "display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;" +
      "align-items:stretch!important;max-width:830px!important;width:max-content!important;" +
      "min-width:560px!important;box-sizing:border-box!important;margin:0!important}" +
      "div.easyLink-box div.easyLink-img{" +
      "margin-right:20px!important;height:240px!important;position:relative!important;" +
      ...
      "width:240px!important;height:240px!important;display:flex!important;" +
      "justify-content:center!important;align-items:center!important;" +
      "margin:0!important;flex:0 0 240px!important}" +
      ...
      "width:auto!important;flex:1 1 auto!important;min-width:280px!important}" +
      "div.easyLink-box div.easyLink-info p.easyLink-info-btn{" +
      "margin-top:20px!important;display:flex!important;flex-wrap:wrap!important;" +
      "justify-content:flex-start!important;gap:10px!important}" +
      "div.easyLink-box div.easyLink-info p.easyLink-info-btn a{" +
      "width:280px!important;max-width:100%!important}" +
      "</style></head><body>" +
      safe +
      "</body></html>";
    iframe.style.cssText = "border:0;display:block;background:transparent;";

    function fitToBlackBox() {
      try {
        const d = iframe.contentDocument;
        if (!d || !d.body) return;
        const box =
          d.querySelector("div.easyLink-box") ||
          d.querySelector('[id^="msmaflink-"]') ||
          d.body;
        const r = box.getBoundingClientRect();
        const w = Math.max(Math.ceil(r.width), box.offsetWidth || 0, 560);
        const h = Math.max(Math.ceil(r.height), box.offsetHeight || 0, 120);
    ...
    iframe.addEventListener("load", function () {
      fitToBlackBox();
      const id = window.setInterval(fitToBlackBox, 400);
      window.setTimeout(function () { window.clearInterval(id); }, 12000);
    });
    window.addEventListener("resize", fitToBlackBox);
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; margin:0; padding:0; }}
    ...
    .aff-moshimo {{
      display:block; width:100% !important; max-width:100%; min-width:0;
      border:0; background:transparent; overflow-x:auto; overflow-y:hidden;
